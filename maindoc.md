...
- Dump all relevant PKM files. List below will be updated as we dump files crossing them out as we go. 
    
- Upscale them as need be. Will grey out the text of already upscaled textures. 
...
5. Repack with Backyard Tools. 
   ...
   ```python .\unpkm.py -o "[Directory you want the file output to]\[Desired name for .PKM file]" "[Directory of dumped PKM file on step 2]" -t 3```
   or
   ```py .\unpkm.py -o "[Directory you want the file output to]\[Desired name for .PKM file]" "[Directory of dumped PKM file on step 2]" -t 3```
...
```
py .\unpkm.py "D:\SteamLibrary\steamapps\common\GG2\data\TD271.PKM"  

py .\unpkm.py -o “C:\Users\asulm\OneDrive\Documents\Overture Modding\Test PKM\KyC1.PKM" "C:\Users\asulm\OneDrive\Documents\Overture Modding\Backyard Tools\TD271" -t 3 -vf

py .\unpkm.py “D:\SteamLibrary\steamapps\common\GG2\data\obj\SMMOT.PKM”

py .\unpkm.py “D:\SteamLibrary\steamapps\common\GG2\data\sound\en\SIN_MASTER_CPACK.PKM” 
```
______________
...
1. Install Python from the website, make sure to check the add to PATH option on the installer

![](https://lh7-us.googleusercontent.com/AE3vR79h87nspEXXManCLPpP03-OaKP19p1WTsYYbyv9NgIgEbMRsg5k_iwb8ZFcBlJc5OBf-dihpTUnnj2ZhuIE67vce25D8Uu108d4EVK42GLGu_KNbrXF4HfSVncRjRzG-D5SYbqna3SU3MtjlKA)

2. Open cmd on the GUI folder (type cmd on the browser bar), then type the following:
   `pip install -r requirements.txt`

3. Once the installation finishes, type the following:
   `py test.py`

The GUI should then launch (You only need to do this once).
...
Everything beyond this point is either the FX or is the stages.
...
#### ⚔️ TD10-14 = Servants and Masterghosts

##### TD10X = Izuna’s Servants
...
##### TD12X = Sol Badguy’s Servants (Both)
...
Namely these DDS files: 130 (Chimaki), 845 (Fence?), 971 + 972 (Chest), 975-976 (Some item?), Lots of Rods all over the place, 985-992 (Scarecrows), Stained Glass?, Some Diamonds???
...
#### ⚔️ TD40X-TD43X = Other Characters
...
OUTGAMERESULT, OUTGAMEBATTLEINTRO, OUTGAME, OUTGAMETITLE, COCKPIT

Most UI is in COCKPIT and OUTGAME, which are in data\cockpit and data\outgame respectively
